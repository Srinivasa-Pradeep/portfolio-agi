...
const companies = [
  { name: 'SAP', imageId: 'sap-logo', className: 'transition-all duration-300 filter grayscale drop-shadow-lg blur-sm scale-75 hover:grayscale-0 hover:blur-none hover:scale-90 hover:drop-shadow-xl' },
  { name: 'Amazon', imageId: 'amazon-logo', className: 'transition-all duration-300 filter grayscale drop-shadow-lg blur-[1.5px] scale-90 hover:grayscale-0 hover:blur-none hover:scale-100 hover:drop-shadow-xl' },
  { name: 'Mercedes-Benz', imageId: 'mercedes-logo', className: 'transition-all duration-300 filter grayscale drop-shadow-lg blur-0 scale-100 hover:grayscale-0 hover:scale-105 hover:drop-shadow-2xl' },
];
              ...
              <TooltipProvider>
                <div className="mt-8 flex items-center justify-center gap-8 md:gap-12">
                  {companies.map((company) => {
                    const logoImage = PlaceHolderImages.find(p => p.id === company.imageId);
                    return (
                ...
                {Object.entries(skills).map(([category, items]) => (
                  <div key={category}>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {items.map((item) => (
                        <Badge key={item} variant="secondary" className="px-3 py-1 text-sm transition-all duration-200 hover:scale-105 hover:shadow-md hover:bg-primary/10">{item}</Badge>
                      ))}
                    </div>
                  </div>
                ))}
