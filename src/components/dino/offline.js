...
import {loadTimeData} from './load_time_data_mock.js';
...
Runner.prototype = {
  ...
  loadImages() {
    ...
    const img = new Image();
    img.src = scale === '2x' ? '/images/200-offline-sprite.png' : '/images/100-offline-sprite.png';
    Runner.imageSprite = img;
    Runner.origImageSprite = img;

    if (this.gameType) {
      Runner.altGameImageSprite = this.createImageElement('altGameSpecificImage' + scale);
      Runner.altCommonImageSprite = this.createImageElement('altGameCommonImage' + scale);
    }
    ...
    if (img.complete) {
      ...
      img.addEventListener(Runner.events.LOAD, this.init.bind(this));
    }
  },
  ...
  loadSounds() {
    ...
    if (this.audioContext) {
      const soundFiles = {
        ...
        HIT: '/sounds/hit.mp3',
        SCORE: '/sounds/score-reached.mp3'
      };
      for (const [soundKey, soundPath] of Object.entries(soundFiles)) {
        fetch(soundPath)
          .then(response => response.arrayBuffer())
          .then(buffer => this.audioContext.decodeAudioData(buffer))
          .then(audioData => {
            this.soundFx[soundKey] = audioData;
          })
          .catch(err => console.error('Failed to load sound:', soundKey, err));
      }
    }
  },
